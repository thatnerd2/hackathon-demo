...
namespace Microsoft.DevAI.ResourceProvider.API.Helpers
{
    ...
    public static class ArmResourceHelpers
    {
        ...
        /// <param name="cancellationToken">The cancellation token.</param>
        public static async Task<BatchResource> CreateBatchAccountAsync(ILogger logger, AccessToken accessToken, UnversionedInstance instance, UnversionedSandbox sandbox, KeyVaultResource keyVault, AppServiceResource appService, IEnumerable<CertificateBundle> certificateBundles, StorageResource metadataStorage, VirtualNetworkResource vNet, UserAssignedManagedIdentityResource userAssignedManagedIdentity, UserAssignedManagedIdentityResource globalUserAssignedManagedIdentity, ArmAuthHelper armAuthHelper, IDictionary<string, string> tags, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();

            using var session = new SessionLogger(
                logger: logger,
                sessionEventName: "batch/create",
                sessionMessage: string.Format(RPConstants.ArmManager.EventMessageCreateOrUpdateTemplate, RPConstants.ResourceTypes.Batch),
                localProperties: new Dictionary<string, string>() {
                    { RPConstants.Telemetry.Property.Certificates, JsonConvert.SerializeObject(certificateBundles.Select(c => new
                        {
                            CertificateName = c.CertificateIdentifier.Name,
                            CertificateIssuerName = c.Policy.IssuerParameters.Name,
                            CertificateSubjectName = c.Policy.X509CertificateProperties.Subject,
                            CertificateThumbprint = new X509Certificate2(c.Cer).Thumbprint
                        }))
                    }
                },
            ...
            try
            {
                // When we create the batch in the instance, we need to create the slots. So we set the active slot to null and that will create both slots.
                // We don't want to create Slots for Sandbox resources. So set the active one as SlotSuffixTwo. That in turn set the target slot to SlotSuffixOne.
                var activeSlot = (sandbox is null) ? null : RPConstants.SlotSuffixTwo;
                var batchName = (sandbox is null) ? instance.Properties.InternalMetadata.AzureResources.Batch.Name : sandbox.Properties.InternalMetadata.AzureResources.Batch.Name;
                var batchAccount = await ArmResourceHelpers.CreateArmResourceAsync<BatchResource, BatchAccount, BatchDependentResources>(
                    batchName,
                    accessToken,
                    instance,
                    logger,
                    tags,
                    ArmResourceHelpers.CreateBatchDependentResources(instance, sandbox, appService, certificateBundles, keyVault, metadataStorage, vNet, userAssignedManagedIdentity, globalUserAssignedManagedIdentity, armAuthHelper, activeSlot),
                    cancellationToken: cancellationToken);

                var batchKey = await batchAccount.GetKeyAsync(cancellationToken: cancellationToken);

                if (sandbox is null)
                {
                    instance.Properties.InternalMetadata.AzureResources.MachinePools = ArmResourceHelpers.AddOrReplaceMachinePool(instance.Properties.InternalMetadata.AzureResources.MachinePools, instance.Properties.NamedSharedMachinePool);
                    await keyVault.CreateSecretAsync(armAuthHelper.GetKeyVaultClient, Common.SecretNames.BatchKey, batchKey, cancellationToken);
                }
                else
                {
                    sandbox.Properties.InternalMetadata.AzureResources.MachinePools = ArmResourceHelpers.AddOrReplaceMachinePool(sandbox.Properties.InternalMetadata.AzureResources.MachinePools, sandbox.Properties.NamedSharedMachinePool);
                    await keyVault.CreateSecretAsync(armAuthHelper.GetKeyVaultClient, RpHelpers.GetSandboxBatchKeySecretName(sandbox.Name), batchKey, cancellationToken);
                }

                session.Properties.Add("batch_account_id", batchAccount.Id);
